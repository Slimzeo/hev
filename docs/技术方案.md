...
感觉该换个名字，skill-env看起来太别扭了，太变态了
*skillbuddy？*

```bash
# env
/skill-env env -c/create <env-name>
/skill-env env -rm/remove <env-name>
/skill-env env list
/skill-env env -rn/rename <old-env-name> <new-env-name>
/skill-env env -a/activate <env-name>
#只能取消当前环境；base取消即退出skill-env模式，回到普通没有skll-env的模式
/skill-env env -d/deactivate 

# skill
/skill-env (skill) list <default Args: cur-env/env-name>
/skill-env skill list -a #查看所有skill
# 没想好分配skill到某个env的cmd
/skill-env skill
# 没想好查看skill归属有的env的cmd
/skill-env skill
# 还有一些功能的cmd没想出来就是如下的相关内容，但是应该先把隔离做出来，就是以上最基本的
# 但是感觉skill-env这个名字有点没品。这个cmd name用外部json配置一下，可以让我们随时改名；平时的开发和skill-env这个名词解耦一下
/skill-env skill 
